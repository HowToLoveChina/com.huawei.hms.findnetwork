...
public class GrsInterceptor implements PluginInterceptor {
    ...
    public Response intercept(Interceptor.Chain chain) {
        ...
        String str = TAG;
        Logger.m32143v(str, "request url is grs schema.");
        GrsManager.getInstance();
        Request requestBuild = request.newBuilder().url(new HttpUrl(GrsManager.parseGrs(url))).build();
        Logger.m32144v(str, "origin url is grs schema and by intercepted,and now request is:%s", requestBuild.toString());
