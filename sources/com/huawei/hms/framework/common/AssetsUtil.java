...
public class AssetsUtil {
    ...
    public static String[] list(final Context context, final String str) {
        ...
        try {
            ...
            Logger.m32145w(TAG, "get local config files from sp task timed out");
            return new String[0];
        } catch (Exception unused2) {
            Logger.m32145w(TAG, "get local config files from sp task occur unknown Exception");
            return new String[0];
        } catch (InterruptedException e10) {
            Logger.m32146w(TAG, "get local config files from sp task interrupted", e10);
            return new String[0];
        } catch (ExecutionException e11) {
            Logger.m32146w(TAG, "get local config files from sp task failed", e11);
            return new String[0];
        } finally {
    ...
    public static InputStream open(Context context, String str) throws IOException {
        ...
        try {
            ...
            Logger.m32139e(TAG, "AssetManager has been destroyed", e10);
            return null;
        }
    }
}
