...
public abstract class HuaweiMobileServicesUtil {
    ...
    public static int isHuaweiMobileServicesAvailable(Context context, int i10) {
        ...
        if (HMSPackageManager.getInstance(context).isUseOldCertificate()) {
            HMSLog.m36986e(TAG, "The CP uses the old certificate to terminate the connection.");
            return 13;
        }
        ...
        if (TextUtils.isEmpty(hMSPackageNameForMultiService)) {
            HMSLog.m36989w(TAG, "hmsPackageName is empty, Service is invalid.");
            return 1;
        }
        HMSLog.m36988i(TAG, "hmsPackageName is " + hMSPackageNameForMultiService);
        ...
        if (packageStates.equals(hMSPackageStatesForMultiService)) {
            HMSLog.m36989w(TAG, "HMS is not installed");
            return 1;
        }
        ...
        HMSLog.m36988i(TAG, "connect versionCode:" + HMSPackageManager.getInstance(context).getHmsMultiServiceVersion());
        if (!HMSPackageManager.getInstance(context).isApkUpdateNecessary(i10)) {
            return 0;
        }
        HMSLog.m36988i(TAG, "The current version does not meet the minimum version requirements");
        return 2;
    }
